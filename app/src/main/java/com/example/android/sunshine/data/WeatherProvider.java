                ...
                try {
                    for (ContentValues value : values) {
                        ...
                        if (!SunshineDateUtils.isDateNormalized(date)) {
                            throw new IllegalArgumentException("Date format is not normalized");
                        }

                        long _id = sqLiteDatabase.insert(WeatherContract.WeatherEntry.TABLE_NAME, null, value);
                        if (_id > 0) {
                            insertCount++;
                        }
                    }
...
//        throw new RuntimeException("Student, you need to implement the delete method!");
        ...
        switch (sUriMatcher.match(uri)) {
            case CODE_WEATHER:
                delCount = sqLiteDatabase.delete(WeatherContract.WeatherEntry.TABLE_NAME, selection, selectionArgs);
                break;
            default:
                throw new UnsupportedOperationException("Delete can not be done for uri -> " + uri);
        }
